...
var App = function() {
     ...
     *
     * App.initHelper('border-radius:5px;s');
     *
     */
    var uiHelperDraggableItems = function(){
        ...
        jQuery('.js-draggable-items > .draggable-column').sortable({
            ...
            receive: function(e){
                //var target_list_id = e.target.id.replace('list-', '');
                //var cards = document.getElementById(e.target.id + '-items').querySelectorAll('div[data-list-id]:not([data-list-id="' + target_list_id + '"])');

                //CardMovedBetweenLists(target_list_id, cards);
            },
            stop: function(e){
                //var target_list_id = e.target.id.replace('list-', '');
                //alert("Card stopped on " + target_list_id);
                //OrderList(target_list_id);
            },
            update: function (e, ui) {
                var card_id = ui.item.attr('data-card-id');
                var list_id = ui.item.attr('data-list-id');
                var target_list_id = e.target.id.replace('list-', '');
                //alert('Updated: ' + String(card_id) + '\n' + 'List: ' + list_id + '\n' + 'Target: ' + target_list_id);

                if (list_id !== target_list_id){
                    CardMovedBetweenLists(card_id, target_list_id);
                }
                else{
                    ReorderList(list_id);
                }
